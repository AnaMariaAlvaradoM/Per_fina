...
import { fmt, fmtRelative, fmtDate } from '../components/ui/helpers.jsx';
...
export default function HogarPage({ onAdd }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api.getTransactions({ household_id: household.id, limit: 50 }),
      api.getAccounts(household.id),
    ]).then(([s, t, a]) => {
      setSummary(s);
      // Solo mostrar transacciones marcadas como compartidas
      setTransactions(t.filter(x => x.household_id));
      setAccounts(a.shared || []);
  ...
  if (!household) return (
    <div style={{ textAlign: 'center', padding: 48, color: 'var(--text2)' }}>
      ...
      <div style={{ fontWeight: 700, marginBottom: 8 }}>Sin hogar compartido</div>
      <div style={{ fontSize: '0.85rem', marginBottom: 20 }}>
        Ve a Cuentas para ver tu código de invitación o unirte al hogar de tu pareja.
      </div>
    </div>
  );
  ...
  return (
    <div className="stack">
      ...
      <div className="flex-between page-header">
        <div>
          <div className="page-title">🏠 {household.name}</div>
      ...
      {/* Código invitación */}
      <div className="card" style={{ borderStyle: 'dashed', borderColor: 'var(--green)', background: 'var(--green-dim)' }}>
      ...
      {/* Resumen del mes */}
      <div className="card" style={{ background: 'linear-gradient(135deg, #0f2a1e, #0a1f18)', border: '1px solid #1a4a30' }}>
        <div style={{ fontSize: '0.72rem', color: '#4ade80', fontWeight: 600, letterSpacing: '0.05em', marginBottom: 8 }}>
          GASTOS COMPARTIDOS — {['ENE','FEB','MAR','ABR','MAY','JUN','JUL','AGO','SEP','OCT','NOV','DIC'][now.getMonth()]} {now.getFullYear()}
        </div>
        <div style={{ display: 'flex', gap: 20 }}>
          ...
          <div>
            <div style={{ fontSize: '0.72rem', color: 'var(--text2)' }}>Balance</div>
          ...
          <div style={{ display: 'flex', gap: 10, overflowX: 'auto', paddingBottom: 4 }}>
            {accounts.map(a => (
              <div key={a.id} className="account-card"
                style={{ minWidth: 155, color: a.color || '#10b981',
            ...
            <ResponsiveContainer width={120} height={120}>
              <PieChart>
                ...
                <Tooltip formatter={(v) => fmt(v)} contentStyle={{
                  background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 8, fontSize: '0.8rem'
                }} />
              </PieChart>
            </ResponsiveContainer>
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 6 }}>
              {expCats.slice(0, 5).map((c, i) => (
                <div key={i} className="flex-between" style={{ fontSize: '0.8rem' }}>
                  ...
                  <span style={{ color: 'var(--text2)', fontFamily: 'var(--mono)', fontSize: '0.78rem' }}>{fmt(c.total)}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div>
        <h3 style={{ marginBottom: 10 }}>Movimientos compartidos</h3>

        {transactions.length === 0 && (
          <div style={{ textAlign: 'center', padding: 32, color: 'var(--text2)' }}>
            <div style={{ fontSize: '1.8rem', marginBottom: 8 }}>📭</div>
            <div style={{ fontSize: '0.85rem', marginBottom: 12 }}>
              Aún no hay gastos compartidos.<br/>Al registrar un movimiento, marca "Gasto compartido del hogar".
            </div>
            <button className="btn btn-primary btn-sm" onClick={onAdd}>+ Primer gasto compartido</button>
          </div>
        )}
              ...
              {items.map((t, i) => (
                <div key={t.id}>
                  ...
                  <div className="tx-item">
                    <div className="tx-icon" style={{ background: `${t.category_color || '#10b981'}22`, flexShrink: 0 }}>
                    ...
                    <div className="tx-info">
                      ...
                      <div className="tx-meta">
                        ...
                        {t.created_by_name && <span style={{ color: 'var(--accent)' }}> · {t.created_by_name}</span>}
                      </div>
                    </div>
